...
public final class WorldModel {
    ...
    public static void parseEntity(WorldModel world, String line, ImageStore imageStore) {
        String[] properties = line.split(" ", Entity.NUM_PROPERTIES + 1);
        if (properties.length >= Entity.NUM_PROPERTIES) {
            ...
            properties = properties.length == Entity.NUM_PROPERTIES ?
                    new String[0] : properties[Entity.NUM_PROPERTIES].split(" ");
    ...
    public void tryAddSlimeEntity(Entity entity, EventScheduler scheduler, ImageStore imageStore){
        if(isOccupied(entity.getPosition())){
            System.out.println("Can't add slime to occupied tile.");
        }
        else{
            this.addEntity(entity);
            ((Slime)entity).scheduleActions(scheduler, this, imageStore);
        }
    }
    ...
    public void parseDude(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Dude.DUDE_NUM_PROPERTIES) {
            Entity entity = new DudeNotFull(id, pt, Double.parseDouble(properties[Dude.DUDE_ACTION_PERIOD]), Double.parseDouble(properties[Dude.DUDE_ANIMATION_PERIOD]),
                    Integer.parseInt(properties[Dude.DUDE_LIMIT]), imageStore.getImageList(Dude.DUDE_KEY));
            this.tryAddEntity(entity);
        }else{
    ...
    public void parseFairy(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Fairy.FAIRY_NUM_PROPERTIES) {
            Entity entity = new Fairy(id, pt, Double.parseDouble(properties[Fairy.FAIRY_ACTION_PERIOD]), Double.parseDouble(properties[Fairy.FAIRY_ANIMATION_PERIOD]),
    ...
    public void parseTree(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Tree.TREE_NUM_PROPERTIES) {
            Entity entity = new Tree(id, pt, Double.parseDouble(properties[Tree.TREE_ACTION_PERIOD]), Double.parseDouble(properties[Tree.TREE_ANIMATION_PERIOD]),
                    Integer.parseInt(properties[Tree.TREE_HEALTH]), imageStore.getImageList(Tree.TREE_KEY));
            this.tryAddEntity(entity);
        }else{
    ...
    public void parseObstacle(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Obstacle.OBSTACLE_NUM_PROPERTIES) {
            Entity entity = new Obstacle(id, pt, Double.parseDouble(properties[Obstacle.OBSTACLE_ANIMATION_PERIOD]),
