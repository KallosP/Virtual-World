...
public final class VirtualWorld extends PApplet {
    ...
    */
    ...
    public List<Point> poisonedPoints;

    public void poisonWorld(Point mClick, WorldModel world, ImageStore imageStore){
        poisonedPoints = mClick.chooseRandomNearbyGrassTile(world);

        for(Point pt : poisonedPoints){
            world.setBackgroundCell(pt, new Background("poisoned_point", new ArrayList<>(imageStore.getImageList("poisoned_grass"))));
        }
    ...
    public void mousePressed() {
        ...
        // Create new slime
        slime = new Slime("", new Point(pressed.getX(), pressed.getY()), imageStore.getImageList(Slime.SLIME_KEY),
                Slime.SLIME_ACTION_PERIOD, Slime.SLIME_ANIMATION_PERIOD);
        // Poison random tiles around click
        poisonWorld(new Point(pressed.getX(), pressed.getY()), world, imageStore);
        // Add slime to the world
        world.tryAddSlimeEntity(slime, scheduler, imageStore);


        Optional<Entity> entityOptional = world.getOccupant(pressed);
        if (entityOptional.isPresent()) {
            Entity entity = entityOptional.get();
            if(entity.getKind().equals(Sapling.class)){
                System.out.println(entity.getId() + ": " + entity.getKind() + " : " + ((Sapling) entity).getHealth());
            }
            else if(entity.getKind().equals(Tree.class)){
                System.out.println(entity.getId() + ": " + entity.getKind() + " : " + ((Tree) entity).getHealth());
            }
            else{
